...
import (
	...
	"github.com/xmapst/mixed-socks/internal/component/dhcp"
...
func (d *dhcpClient) resolve(ctx context.Context) ([]dnsClient, error) {
	...
	if err != nil {
		...
		go func() {
			...
			var res []dnsClient
			_dns, err := dhcp.ResolveDNSFromDHCP(ctx, d.ifaceName)
			// dns never empty if err is nil
			if err == nil {
				nameserver := make([]NameServer, 0, len(_dns))
				for _, item := range _dns {
